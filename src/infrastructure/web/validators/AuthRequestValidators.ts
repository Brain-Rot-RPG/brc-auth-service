...
export class AuthRequestValidators {
    ...
    static register(body: any): void {
        // Required fields
        if (!body) throw new InvalidDataError('body', 'Missing request body');
        ...
        if (username.length < 3 || username.length > 20) {
            throw new InvalidDataError('username', 'Must be between 3 and 20 characters');
        }
        if (!/^[a-zA-Z0-9_]+$/.test(username)) {
            throw new InvalidDataError('username', 'Must contain only letters, numbers and underscores');
        }
        if (password.length < 12) {
            throw new InvalidDataError('password', 'Must be at least 12 characters');
        }
    }
    ...
    static updateProfile(body: any): void {
        ...
        Object.keys(body).forEach(key => {
            if (!allowed.includes(key)) {
                throw new InvalidDataError(key, 'Field not allowed');
            }
        });
        
        if (Object.keys(body).length === 0) {
            throw new InvalidDataError('body', 'At least one field must be provided');
        }
    }
}
